...
/**
 * In this class functions for implementation of graphs are defined.
 */
	...
	public void printGraph() {
		for (int i = 0; i < numberOfVertices; i++) {
			...
			for (int j = 0; j < edges.size(); j++) {
				System.out.println("vertex-" + i + " is connected to " + edges.get(j).getDestination() + " with weight " + edges.get(j).getWeight());
			}
		}
	}
			...
			for (int v = 0; v < numberOfVertices; v++)
				...
				for (EdgeAndVertices listEdges : edges) {
					...
					if (listEdges.getDestination() == v) {
						...
						if (!sptSet[v] && listEdges.getWeight() != 0 && dist[u] != Integer.MAX_VALUE  && dist[u] + listEdges.getWeight() < dist[v]) {
