...
import com.metacube.MetacubeParkingSpring.Service.EmployeeService;
...
public class EmployeeController {
	...
	public String registerEmployee(@Valid @ModelAttribute("employee") Employee employee, Errors errors, Model model) {
		if (errors.hasErrors()) {
			...
			if (empId == -1) {
				...
				System.out.println("NEW EMP ID: "+employeeId);
	...
	public String loginEmployee(@Valid @ModelAttribute("employeeDto") EmployeeLogin employeeDto, Errors errors,
			HttpSession session) {
		if (errors.hasErrors()) {
			return "login";
		} else {
